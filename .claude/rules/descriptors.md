...
- `{{DIR}}` receives a directory **name**, so the value template must match it at any nesting level (`**/{{DIR}}/**`, `(^|/){{DIR}}/`, `*/{{DIR}}/*`). A root-anchored template (`./{{DIR}}`, `{{DIR}}/**`) silently misses nested occurrences. A template containing `{{WORKSPACE}}` is the exception: it needs a real path, so `{{DIR}}` is replaced by each workspace-relative path actually found (`get_project_exclude_directory_paths`).
