...
| `cli_lint_mode_project_exclude_arg_name`  | Native CLI flag forwarding `EXCLUDED_DIRECTORIES` in `project` mode (e.g., `--ignore-pattern`, `--exclude`, `--ignore=`). CLI flag only — not an ignore file     |
| `cli_lint_mode_project_exclude_arg_value` | Value template, `{{DIR}}` = directory name (default `{{DIR}}`; e.g., `**/{{DIR}}/**`)                                                                            |
| `cli_lint_mode_project_exclude_separator` | Set ONLY if the flag is not repeatable — joins all dirs into one occurrence (e.g., `,`, `\|`)                                                                    |
...
| `cli_lint_mode_project_exclude_ignore_file_arg_name` | Flag receiving a generated ignore file (prettier/markdownlint `--ignore-path`); with `_seed_files` (workspace files merged in, first existing wins), `_pass_existing` (files re-passed when the flag replaces their discovery), `_skip_if_config` |
