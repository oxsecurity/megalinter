...
def generate_flavor_documentation(flavor_id, flavor, linters_tables_md):
    flavor_github_action = f"{ML_REPO}/flavors/{flavor_id}@v4"
    flavor_docker_image = f"{ML_DOCKER_IMAGE}-{flavor_id}:v4"
    docker_image_badge = (
        f"![Docker Image Size (tag)]({BASE_SHIELD_IMAGE_LINK}/"
        f"{ML_DOCKER_IMAGE}-{flavor_id}/v4)"
    )
    docker_pulls_badge = (
...
def build_flavors_md_table(filter_linter_name=None, replace_link=False):
    ...
    docker_image_badge = (
        f"![Docker Image Size (tag)]({BASE_SHIELD_IMAGE_LINK}/{ML_DOCKER_IMAGE}/v4)"
    )
    docker_pulls_badge = (
