...
def get_badges(
    ...
    if (show_downgraded_version and 
        (hasattr(linter, "get") and linter.get("downgraded_version") is True) or (
        hasattr(linter, "downgraded_version") and linter.downgraded_version is True)
    ):
        badges += ["![downgraded version](https://shields.io/badge/-downgraded%20version-orange)"]
    if repo is not None:
        badges += [
            f"[![GitHub stars](https://img.shields.io/github/stars/{repo}?cacheSeconds=3600)]"
