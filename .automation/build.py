...
def generate_all_flavors():
    ...
    for flavor, flavor_info in flavors.items():
        generate_flavor(flavor, flavor_info, "Dockerfile")
        generate_flavor(flavor, flavor_info, "Dockerfile_arm")
...
def generate_flavor(flavor, flavor_info, dockerFileName):
    ...
    if flavor == "all":
        dockerfile = f"{REPO_HOME}/{dockerFileName}"
        if RELEASE is True:
