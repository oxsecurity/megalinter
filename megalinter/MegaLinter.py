...
def run_linters(linters,request_id):
    global RUN_CONFIGS
    config.set_config(request_id,RUN_CONFIGS) #Avoid to loose global variable because of multiprocessing
...
class Megalinter:
    ...
    def process_linters_parallel(self, active_linters, linters_do_fixes):
        ...
        install_mp_handler()
        pool = mp.Pool(process_number, initializer=init_worker, initargs=(config.get(self.request_id),))
        pool_results = []
