        ...
        if config.get("GITHUB_TOKEN", "") != "":
            ...
            for pr in pr_list:
                ...
                try:
                    ...
                    logging.info(
                        f"[GitHucCommentReporter] Posted summary as comment on {github_repo} #PR{pr.number}"
                    )
                    posted = True
                except github.GithubException as e:
                    logging.warning(
                        f"[GitHucCommentReporter] Unable to post pull request comment: {str(e)}.\n"
            ...
            if posted is False:
                logging.warning(
                    "[GitHucCommentReporter] No pull request was found, so no comment has been posted"
                )
        # Not in github context, or env var POST_GITHUB_COMMENT = false
        else:
            logging.warning(
                "[GitHucCommentReporter] No GitHub Token found, so skipped post of PR comment"
            )
