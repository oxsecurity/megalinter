    ...
    def produce_report(self):
        ...
        for updated_file in updated_files:
            ...
            try:
                ...
                logging.warning(
                    f"[UpdatedSourcesReporter] Unable to copy {source_file} to {target_file} ({str(copy_err)})"
                )
        # Log
        if len(updated_files) > 0:
            logging.info(
                f"[UpdatedSourcesReporter] copied {str(len(updated_files))} fixed source files"
                f" in folder {updated_sources_dir}.\n"
                "Download it from artifacts then copy-paste it in your local repo to apply linters updates"
            )
        else:
            logging.info(
                "[UpdatedSourcesReporter] No source file has been formatted or fixed"
            )
