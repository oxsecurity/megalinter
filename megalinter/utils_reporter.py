...
import requests
from megalinter import ci_providers, config, utils
...
def log_section_start(section_key: str, section_title: str):
    if _ci_sections_enabled():
        return ci_providers.get_ci_provider(None).log_section_start(
...
def log_section_end(section_key):
    if _ci_sections_enabled():
        return ci_providers.get_ci_provider(None).log_section_end(section_key)
    return ""
