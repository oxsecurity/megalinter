        ...
        for linter_descriptor in language_descriptor.get('linters'):
            ...
            if linter_descriptor.get('class'):
                linter_class_file_name = os.path.splitext(
                    os.path.basename(linter_descriptor.get('class')))[0]
            ...
            linter_instance = linter_class(
                linter_init_params, instance_attributes)
...
def build_linter(language, linter_name):
    ...
    assert len(
        linters) == 1, f"Unable to find linter {linter_name} in {language_descriptor_file}"
    return linters[0]
...
def list_active_reporters_for_scope(scope, reporter_init_params):
    ...
    reporters_dir = os.path.realpath(os.path.dirname(os.path.abspath(__file__)) + '/reporters')
    scope_reporters = []
    for reporter_class_file in os.listdir(reporters_dir):
        ...
        reporter_module = importlib.import_module('.reporters.' + reporter_class_nm,
                                                  package=__package__)
