...
class GitleaksLinter(Linter):
    def __init__(self, params=None, linter_config=None):
        ...
        self.pr_commits_scan = config.get(self.request_id, "REPOSITORY_GITLEAKS_PR_COMMITS_SCAN", "false")
    ...
    def get_pr_data(self):
        ...
        if pr_source_sha is None or pr_target_sha is None:
            if utils.is_azure_devops_pr():
                ...
                pr_source_sha = config.get(self.request_id, "SYSTEM_PULLREQUEST_SOURCECOMMITID")
                # SYSTEM_PULLREQUEST_TARGETBRANCH -> name of the target branch, e.g. refs/heads/main
                pr_target_sha = self.get_azure_devops_pr_target_sha(config.get(self.request_id, "SYSTEM_PULLREQUEST_TARGETBRANCH"))
                ...
                pr_source_sha = config.get(self.request_id, "CI_MERGE_REQUEST_SOURCE_BRANCH_SHA")
                # CI_MERGE_REQUEST_TARGET_BRANCH_SHA -> SHA of the last commit in the target branch
                pr_target_sha = config.get(self.request_id, "CI_MERGE_REQUEST_TARGET_BRANCH_SHA")
                ...
                pr_source_sha = config.get(self.request_id, "CI_EXTERNAL_PULL_REQUEST_SOURCE_BRANCH_SHA")
                # CI_EXTERNAL_PULL_REQUEST_TARGET_BRANCH_SHA -> SHA of the last commit in the target branch
                pr_target_sha = config.get(self.request_id, "CI_EXTERNAL_PULL_REQUEST_TARGET_BRANCH_SHA")
    ...
    def build_lint_command(self, file=None):
        ...
        if "--redact" in self.cli_lint_user_args or "--no-git" in self.cli_lint_user_args:
        ...
        if config.get(self.request_id, "VALIDATE_ALL_CODEBASE") == "false" and self.pr_commits_scan == "true" and utils.is_pr():
