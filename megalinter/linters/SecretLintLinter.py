...
class SecretLintLinter(Linter):
    ...
    # patterns, the excluded directories forwarded to every project mode linter,
    # and MegaLinter's report folder. It lives in the report folder, not the
    # workspace, so MegaLinter never creates or removes
    ...
    def build_megalinter_ignore_file(self):
        ...
        # Forwarded the same way as for every other project mode linter, but merged
        # here rather than into a second ignore file at the workspace root: only
        # directories that exist at the workspace root are listed, and the user can
        # opt out with <LINTER_KEY>_FORWARD_EXCLUDED_DIRECTORIES
        if self.is_project_exclude_forwarding_active():
            forwarded = [
                excluded_dir.replace(os.path.sep, "/")
                for excluded_dir in self.get_project_exclude_directories()
            ]
            # The report folder is an excluded directory too, and gets its own
            # authoritative line below
            forwarded = [
                excluded_dir
                for excluded_dir in forwarded
                if excluded_dir != report_folder_pattern
            ]
            if len(forwarded) > 0:
                lines += ["# Added by MegaLinter: EXCLUDED_DIRECTORIES"]
                lines += forwarded
                self.log_project_exclude_forwarding(
                    f"Merged EXCLUDED_DIRECTORIES into {ignore_file_path} "
    ...
    # The generic cli_lint_mode_project_exclude_ignore_file_* forwarding is not
    # used for secretlint: it would write a second ignore file at the workspace
    # root and pass it as a second --secretlintignore, of which secretlint keeps
    # only the last. The excluded directories are merged into the generated file
    # by build_megalinter_ignore_file instead, so a single file carries every
    # pattern and nothing is written to the linted sources (#3979).
