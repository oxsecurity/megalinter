...
"""
        ...
        cspell_config_file = reporter_self.master.github_workspace + os.path.sep+reporter_self.master.config_file_name
        if os.path.isfile(cspell_config_file):
            ...
            data = cspell_example
        proposed_cspell_config_file = reporter_self.report_folder + os.path.sep + reporter_self.master.config_file_name
        with open(proposed_cspell_config_file, "w", encoding="utf-8") as outfile:
            json.dump(data, outfile, indent=4, sort_keys=True)
        proposed_cspell_config_file = utils.normalize_log_string(proposed_cspell_config_file)
        additional_report += "\n".join([
            "",
