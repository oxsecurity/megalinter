...
def load_plugin(plugin):
    if plugin.startswith("https://"):
        ...
        if not plugin.contains("/mega-linter-plugin-"):
            raise Exception("[Plugins] Plugin descriptor file must be hosted in a directory containing /mega-linter-plugin-")
        if not descriptor_file.endswith(".mega-linter-descriptor.yml"):
            raise Exception("[Plugins] Plugin descriptor file must end with .mega-linter-descriptor.yml")
        ...
        try:
            ...
            logging.info(f"[Plugins] Loaded plugin descriptor {descriptor_file} from {plugin}")
        except Exception as e:
            raise Exception(f"[Plugins] Unable to load plugin {plugin}:\n{str(e)}")
        return descriptor_file
    else:
        raise Exception("[Plugins] Plugin descriptors must follow the format"
...
def install_plugin(descriptor_file):
    ...
    logging.info(f"[Plugins] Successful initialization of {descriptor['descriptor_id']} plugins")
...
def process_install(install):
    ...
    if 'dockerfile' in install:
        ...
        commands += [command.replace("RUN ").replace(" \\\n", "\n") for command in install['dockerfile']]
    ...
    for command in commands:
        ...
        if return_code != 0:
            raise Exception(f'[Plugins] Error while running install command {command}:\n{stdout}')
