...
class ReviewdogLinterReporterRegexConvertorTestCase(unittest.TestCase):
    ...
    def test_regex_convertor(self, linter, mock_report_file):
        ...
        linter.stdout = "\n".join([
            ...
            "/a/different/path/file.py:2:1:Why would you do that?"
        ])

        reporter = ReviewdogLinterReporter({"master": linter, "report_folder": "/tmp/reports"})
        reporter.produce_report()

        mock_report_file.assert_called_once_with("/tmp/reports/reviewdog/ERROR-PYTHON_PYLINT.log", "w", encoding="utf-8")
        mock_report_file().write.assert_called_once_with("\n".join([
            r'{"message": "Something is wrong with this file", "location": {"range": {"start": {"line": 12, "column": 3}, "end": null}, "path": "/some/path.py"}, "severity": "ERROR", "suggestions": []}',
            r'{"message": "Why would you do that?", "location": {"range": {"start": {"line": 2, "column": 1}, "end": null}, "path": "/a/different/path/file.py"}, "severity": "ERROR", "suggestions": []}']) + "\n"
        )
...
"""
        ...
        mock_report_file().write.assert_called_once_with(
            '''{"message": "ERROR: .automation/test/python/python_bad_1.py Imports are incorrectly sorted and/or formatted.", "location": {"range": {"start": {"line": 1, "column": 0}, "end": {"line": 12, "column": 69}}, "path": ".automation/test/python/python_bad_1.py"}, "severity": "ERROR", "suggestions": [{"text": "import json\\nimport sys\\nfrom os import getenv, path\\nfrom pprint import pprint\\n\\nimport click  # pylint: disable=import-error\\nimport requests  # pylint: disable=import-error\\nfrom dotenv import load_dotenv  # pylint: disable=import-error\\n\\nenv = load_dotenv()\\napi_url = getenv(API_URL, default=\'https://api.github.com/graphql\' )", "range": {"start": {"line": 1, "column": 0}, "end": {"line": 12, "column": 69}}}]}\n'''
        )
