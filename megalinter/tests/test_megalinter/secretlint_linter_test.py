...
class SecretLintLinterTest(IsolatedConfigTestCase):
    ...
    def test_ignore_file_written_with_relative_path(self):
        with tempfile.TemporaryDirectory() as workspace:
            ...
            self.assertTrue(
                os.path.isfile(os.path.join(report_folder, MEGALINTER_IGNORE_FILE_NAME))
            )
    ...
    def test_report_folder_is_excluded(self):
        with tempfile.TemporaryDirectory() as workspace:
            ...
            # Only the report folder is added. MegaLinter's wider excluded
            # directories must NOT be, because secretlint is a secrets scanner and
            # build artifacts such as .terraform or .terragrunt-cache can contain
            # real credentials baked in at generate time
    ...
    def test_user_patterns_are_merged_before_megalinter_patterns(self):
        with tempfile.TemporaryDirectory() as workspace:
            ...
            linter = _make_linter(
                workspace, report_folder, self.request_id, ignore_file=user_ignore
            )
    ...
    def test_unreadable_source_ignore_file_degrades(self):
        ...
        with tempfile.TemporaryDirectory() as workspace:
            ...
            generated_file = os.path.join(report_folder, MEGALINTER_IGNORE_FILE_NAME)
            self.assertTrue(os.path.isfile(generated_file))
            with open(generated_file, "r", encoding="utf-8") as fh:
                content = fh.read()
            self.assertIn("\nmegalinter-reports\n", content)
    ...
    def _generated_content_for_user_ignore_arg(self, workspace, user_args):
        ...
        with open(
            ...
            fh.write("vendor\n")
        linter = _make_linter(
            workspace, report_folder, self.request_id, user_args=user_args
        )
    ...
    def test_custom_report_folder_is_excluded(self):
        with tempfile.TemporaryDirectory() as workspace:
            report_folder = os.path.join(workspace, "build", "ml-reports")
            linter = _make_linter(workspace, report_folder, self.request_id)
    ...
    def test_report_folder_outside_workspace_adds_no_parent_pattern(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            report_folder = os.path.join(root, "reports")
