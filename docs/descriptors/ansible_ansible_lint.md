...
- See [How to configure ansible-lint rules](https://ansible-lint.readthedocs.io/en/latest/configuring.html#configuration-file){target=_blank}
  - If custom `.ansible-lint.yml` config file is not found, [.ansible-lint.yml](https://github.com/megalinter/megalinter/tree/master/TEMPLATES/.ansible-lint.yml){target=_blank} will be used
