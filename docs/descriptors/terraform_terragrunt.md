...
- Enable terragrunt by adding `TERRAFORM_TERRAGRUNT` in [ENABLE_LINTERS variable](https://megalinter.io/7.11.1/configuration/#activation-and-deactivation)
- Disable terragrunt by adding `TERRAFORM_TERRAGRUNT` in [DISABLE_LINTERS variable](https://megalinter.io/7.11.1/configuration/#activation-and-deactivation)

- Enable **autofixes** by adding `TERRAFORM_TERRAGRUNT` in [APPLY_FIXES variable](https://megalinter.io/7.11.1/configuration/#apply-fixes)
...
GLOBAL OPTIONS:
   ...
   --terragrunt-provider-cache-disable-partial-lock-file  Don't use 'plugin_cache_may_break_dependency_lock_file' with Terragrunt provider caching. Provider downloads for modules without lock files will be much slower. [$TERRAGRUNT_PROVIDER_CACHE_DISABLE_PARTIAL_LOCK_FILE]
