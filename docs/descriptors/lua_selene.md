...
| LUA_SELENE_FILTER_REGEX_INCLUDE        | Custom regex including filter<br/>Ex: `(src\|lib)`<br/>⚠️ Not available with LUA_SELENE_CLI_LINT_MODE = project                                                                                                     | Exclude no file                                 |
| LUA_SELENE_FILTER_REGEX_EXCLUDE        | Custom regex excluding filter<br/>Ex: `(test\|examples)`<br/>⚠️ Not available with LUA_SELENE_CLI_LINT_MODE = project                                                                                               | Exclude no file                                 |
| LUA_SELENE_CLI_LINT_MODE               | Override default CLI lint mode<br/>- `file`: Calls the linter for each file<br/>- `list_of_files`: Call the linter with the list of files as argument<br/>- `project`: Call the linter from the root of the project | `list_of_files`                                 |
...
selene is called once on the whole project directory (`project` CLI lint mode)

- filtering can not be done using MegaLinter configuration variables,it must be done using selene configuration or ignore file (if existing)
- `VALIDATE_ALL_CODEBASE: false` doesn't make selene analyze only updated files
