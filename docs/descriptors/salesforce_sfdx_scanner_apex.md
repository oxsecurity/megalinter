...
_Disabled reason: sfdx-scanner is incompatible with Node.js 22+ (TypeError: Cannot read properties of undefined (reading 'prototype')). Use SALESFORCE_CODE_ANALYZER_APEX instead._
...
| SALESFORCE_SFDX_SCANNER_APEX_FILTER_REGEX_INCLUDE | Custom regex including filter<br/>Ex: `(src\|lib)`<br/>⚠️ Not available with SALESFORCE_SFDX_SCANNER_APEX_CLI_LINT_MODE = project 
| SALESFORCE_SFDX_SCANNER_APEX_FILTER_REGEX_EXCLUDE | Custom regex excluding filter<br/>Ex: `(test\|examples)` <br/>⚠️ Not available with SALESFORCE_SFDX_SCANNER_APEX_CLI_LINT_MODE = project 
| SALESFORCE_SFDX_SCANNER_APEX_CLI_LINT_MODE | Override default CLI lint mode<br/><- `project`: Call the linter from the root of the projectb- `project`: Call the linter from the root of the projectr- `project`: Call the linter from the root of the project/- `project`: Call the linter from the root of the project> | `project` |
...
```shell
 ›   Warning: @salesforce/cli update available from 2.137.7 to 2.138.6.
...
### SALESFORCE_SFDX_SCANNER_APEX_ERROR_INVALID_PMD_RULESET

**Detection pattern (regex):**

```text
(Cannot load ruleset|RuleSetNotFoundException|PMD failed to load|Invalid ruleset)
```

**Resolution guidance:**

```text
PMD could not load the provided Apex ruleset.
Verify the file referenced by `apex-pmd-ruleset.xml` exists and is valid PMD XML.
You can generate a starter ruleset with: sf scanner:rule:list --engine pmd
```
