...
- If MegaLinter with gitleaks runs against a PR on a platform not listed above, then the analysis is performed on the whole repository - default gitleaks behavior (checked-out commits, depends on fetch-depth configuration).
