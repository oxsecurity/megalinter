...
> If `tflint --init` can not download plugins (for example when `GITHUB_TOKEN` targets a GitHub Enterprise instance and not github.com), use the tflint native host-specific token variable `GITHUB_TOKEN_github_com`: create a github.com Personal Access Token, set it in `GITHUB_TOKEN_github_com`, then allow tflint to read it.

> ```yaml
> TERRAFORM_TFLINT_UNSECURED_ENV_VARIABLES:
>   - GITHUB_TOKEN_github_com
> ```

> tflint gives priority to `GITHUB_TOKEN_{host}` over `GITHUB_TOKEN`, so other linters and reporters keep using your regular `GITHUB_TOKEN`.

> `PAT_GITHUB_COM` does the same thing and is **deprecated**: it will be removed in a future major release, use `GITHUB_TOKEN_github_com` instead.
...
- Enable tflint by adding `TERRAFORM_TFLINT` in [ENABLE_LINTERS variable](https://megalinter.io/10.1.0/configuration/#activation-and-deactivation)
- Disable tflint by adding `TERRAFORM_TFLINT` in [DISABLE_LINTERS variable](https://megalinter.io/10.1.0/configuration/#activation-and-deactivation)
...
| PAT_GITHUB_COM                                | DEPRECATED: github.com Personal Access Token copied into GITHUB_TOKEN when running **tflint --init**. Use tflint native variable `GITHUB_TOKEN_github_com` instead (with `TERRAFORM_TFLINT_UNSECURED_ENV_VARIABLES`), as PAT_GITHUB_COM will be removed in a future major release. | ``                                              |
...
| TERRAFORM_TFLINT_FILTER_REGEX_INCLUDE         | Custom regex including filter<br/>Ex: `(src\|lib)`<br/>⚠️ Not available with TERRAFORM_TFLINT_CLI_LINT_MODE = project                                                                                                                                                              | Exclude no file                                 |
| TERRAFORM_TFLINT_FILTER_REGEX_EXCLUDE         | Custom regex excluding filter<br/>Ex: `(test\|examples)`<br/>⚠️ Not available with TERRAFORM_TFLINT_CLI_LINT_MODE = project                                                                                                                                                        | Exclude no file                                 |
...
| TERRAFORM_TFLINT_FORWARD_EXCLUDED_DIRECTORIES | In project CLI lint mode, forward excluded directories (EXCLUDED_DIRECTORIES + ADDITIONAL_EXCLUDED_DIRECTORIES) to the linter through its native exclusion arguments or generated ignore/config files                                                                              | `true`                                          |
...
| TERRAFORM_TFLINT_TIMEOUT_SECONDS              | Maximum duration in seconds of the linter run, after which the linter process and its child processes are killed and reported as an error (exit code 124). Overrides LINTER_TIMEOUT_SECONDS. 0 disables the timeout                                                                | `300`                                           |
