...
_Disabled reason: Upstream repo archived by Tenable (unmaintained); ships unpatched CVEs (e.g. CVE-2026-33186 grpc, plus a Go-stdlib tail) with no prospect of a fixed release. <https://github.com/tenable/terrascan>_
...
| TERRAFORM_TERRASCAN_FILTER_REGEX_INCLUDE        | Custom regex including filter<br/>Ex: `(src\|lib)`<br/>⚠️ Not available with TERRAFORM_TERRASCAN_CLI_LINT_MODE = project                                                                                            | Exclude no file                                 |
| TERRAFORM_TERRASCAN_FILTER_REGEX_EXCLUDE        | Custom regex excluding filter<br/>Ex: `(test\|examples)`<br/>⚠️ Not available with TERRAFORM_TERRASCAN_CLI_LINT_MODE = project                                                                                      | Exclude no file                                 |
| TERRAFORM_TERRASCAN_CLI_LINT_MODE               | Override default CLI lint mode<br/>- `file`: Calls the linter for each file<br/>- `list_of_files`: Call the linter with the list of files as argument<br/>- `project`: Call the linter from the root of the project | `project`                                       |
