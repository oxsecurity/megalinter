...
_Disabled reason: sfdx-scanner is incompatible with Node.js 22+ (TypeError: Cannot read properties of undefined (reading 'prototype')). Use SALESFORCE_CODE_ANALYZER_LWC instead._
...
| SALESFORCE_SFDX_SCANNER_LWC_FILTER_REGEX_INCLUDE        | Custom regex including filter<br/>Ex: `(src\|lib)`<br/>⚠️ Not available with SALESFORCE_SFDX_SCANNER_LWC_CLI_LINT_MODE = project       | Exclude no file                                 |
| SALESFORCE_SFDX_SCANNER_LWC_FILTER_REGEX_EXCLUDE        | Custom regex excluding filter<br/>Ex: `(test\|examples)`<br/>⚠️ Not available with SALESFORCE_SFDX_SCANNER_LWC_CLI_LINT_MODE = project | Exclude no file                                 |
