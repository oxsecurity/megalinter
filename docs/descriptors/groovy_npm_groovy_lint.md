...
- Enable npm-groovy-lint by adding `GROOVY_NPM_GROOVY_LINT` in [ENABLE_LINTERS variable](https://megalinter.io/10.1.0/configuration/#activation-and-deactivation)
- Disable npm-groovy-lint by adding `GROOVY_NPM_GROOVY_LINT` in [DISABLE_LINTERS variable](https://megalinter.io/10.1.0/configuration/#activation-and-deactivation)

- Enable **autofixes** by adding `GROOVY_NPM_GROOVY_LINT` in [APPLY_FIXES variable](https://megalinter.io/10.1.0/configuration/#apply-fixes)
...
| GROOVY_NPM_GROOVY_LINT_FILTER_REGEX_INCLUDE         | Custom regex including filter<br/>Ex: `(src\|lib)`<br/>⚠️ Not available with GROOVY_NPM_GROOVY_LINT_CLI_LINT_MODE = project                                                                                         | Exclude no file                                 |
| GROOVY_NPM_GROOVY_LINT_FILTER_REGEX_EXCLUDE         | Custom regex excluding filter<br/>Ex: `(test\|examples)`<br/>⚠️ Not available with GROOVY_NPM_GROOVY_LINT_CLI_LINT_MODE = project                                                                                   | Exclude no file                                 |
...
| GROOVY_NPM_GROOVY_LINT_FORWARD_EXCLUDED_DIRECTORIES | In project CLI lint mode, forward excluded directories (EXCLUDED_DIRECTORIES + ADDITIONAL_EXCLUDED_DIRECTORIES) to the linter through its native exclusion arguments or generated ignore/config files               | `true`                                          |
...
| GROOVY_NPM_GROOVY_LINT_TIMEOUT_SECONDS              | Maximum duration in seconds of the linter run, after which the linter process and its child processes are killed and reported as an error (exit code 124). Overrides LINTER_TIMEOUT_SECONDS. 0 disables the timeout | `300`                                           |
...
npm-groovy-lint is called once on the whole project directory (`project` CLI lint mode)

- filtering can not be done using MegaLinter configuration variables,it must be done using npm-groovy-lint configuration or ignore file (if existing)
- `VALIDATE_ALL_CODEBASE: false` doesn't make npm-groovy-lint analyze only updated files
...
- NPM packages (node.js):
  - [npm-groovy-lint@18.0.0](https://www.npmjs.com/package/npm-groovy-lint/v/18.0.0)
