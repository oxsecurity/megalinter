...
- Enable gherkin-lint by adding `GHERKIN_GHERKIN_LINT` in [ENABLE_LINTERS variable](https://megalinter.io/10.1.0/configuration/#activation-and-deactivation)
- Disable gherkin-lint by adding `GHERKIN_GHERKIN_LINT` in [DISABLE_LINTERS variable](https://megalinter.io/10.1.0/configuration/#activation-and-deactivation)
...
| GHERKIN_GHERKIN_LINT_FILTER_REGEX_INCLUDE         | Custom regex including filter<br/>Ex: `(src\|lib)`<br/>⚠️ Not available with GHERKIN_GHERKIN_LINT_CLI_LINT_MODE = project                                                                                           | Exclude no file                                 |
| GHERKIN_GHERKIN_LINT_FILTER_REGEX_EXCLUDE         | Custom regex excluding filter<br/>Ex: `(test\|examples)`<br/>⚠️ Not available with GHERKIN_GHERKIN_LINT_CLI_LINT_MODE = project                                                                                     | Exclude no file                                 |
| GHERKIN_GHERKIN_LINT_CLI_LINT_MODE                | Override default CLI lint mode<br/>- `file`: Calls the linter for each file<br/>- `list_of_files`: Call the linter with the list of files as argument<br/>- `project`: Call the linter from the root of the project | `list_of_files`                                 |
| GHERKIN_GHERKIN_LINT_FORWARD_EXCLUDED_DIRECTORIES | In project CLI lint mode, forward excluded directories (EXCLUDED_DIRECTORIES + ADDITIONAL_EXCLUDED_DIRECTORIES) to the linter through its native exclusion arguments or generated ignore/config files               | `true`                                          |
...
| GHERKIN_GHERKIN_LINT_TIMEOUT_SECONDS              | Maximum duration in seconds of the linter run, after which the linter process and its child processes are killed and reported as an error (exit code 124). Overrides LINTER_TIMEOUT_SECONDS. 0 disables the timeout | `300`                                           |
...
gherkin-lint is called once on the whole project directory (`project` CLI lint mode)

- filtering can not be done using MegaLinter configuration variables,it must be done using gherkin-lint configuration or ignore file (if existing)
- `VALIDATE_ALL_CODEBASE: false` doesn't make gherkin-lint analyze only updated files
