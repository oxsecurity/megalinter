...
| TSX_ESLINT_FILE_NAME | eslint configuration file name</br>Use `LINTER_DEFAULT` to let the linter find it | `.eslintrc.yml` |
...
```shell
eslint --fix -c .eslintrc.yml  --no-eslintrc --no-ignore myfile.tsx
```
