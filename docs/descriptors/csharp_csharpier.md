...
- Enable csharpier by adding `CSHARP_CSHARPIER` in [ENABLE_LINTERS variable](https://megalinter.io/7.8.0/configuration/#activation-and-deactivation)
- Disable csharpier by adding `CSHARP_CSHARPIER` in [DISABLE_LINTERS variable](https://megalinter.io/7.8.0/configuration/#activation-and-deactivation)

- Enable **autofixes** by adding `CSHARP_CSHARPIER` in [APPLY_FIXES variable](https://megalinter.io/7.8.0/configuration/#apply-fixes)
...
Options:
  ...
  --grpc                       Run CSharpier as a service using GRPC so that multiple files may be formatted.
  --grpc-port <grpc-port>      Specify the port that CSharpier should start on. Defaults to a random unused port.
