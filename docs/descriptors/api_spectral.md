...
- Enable spectral by adding `API_SPECTRAL` in [ENABLE_LINTERS variable](https://megalinter.io/10.1.0/configuration/#activation-and-deactivation)
- Disable spectral by adding `API_SPECTRAL` in [DISABLE_LINTERS variable](https://megalinter.io/10.1.0/configuration/#activation-and-deactivation)
...
| API_SPECTRAL_FILTER_REGEX_INCLUDE | Custom regex including filter<br/>Ex: `(src\|lib)` | Exclude no file |
| API_SPECTRAL_FILTER_REGEX_EXCLUDE | Custom regex excluding filter<br/>Ex: `(test\|examples)` | Exclude no file |
| API_SPECTRAL_CLI_LINT_MODE | Override default CLI lint mode<br/>- `file`: Calls the linter for each file<br/>- `list_of_files`: Call the linter with the list of files as argument | `list_of_files` |
...
| API_SPECTRAL_TIMEOUT_SECONDS | Maximum duration in seconds of the linter run, after which the linter process and its child processes are killed and reported as an error (exit code 124). Overrides LINTER_TIMEOUT_SECONDS. 0 disables the timeout | `300` |
...
RUN npm --no-cache install --ignore-scripts --omit=dev --prefix /node-deps-spectral @stoplight/spectral-cli@${NPM_SPECTRAL_CLI_VERSION} \
...
- APK packages (Linux):
  - [npm](https://pkgs.alpinelinux.org/packages?branch=v3.24&arch=x86_64&name=npm)
  - [nodejs-current](https://pkgs.alpinelinux.org/packages?branch=v3.24&arch=x86_64&name=nodejs-current)
