...
- Auto-fix support: **yes** — add `PYTHON_BLACK` (or `all`) to the `APPLY_FIXES` variable, or run locally `npx mega-linter-runner@beta --linter PYTHON_BLACK --fix --release beta` (standalone linter images are only multi-arch on `beta` until MegaLinter v10 is released, and a beta MegaLinter version is driven by the matching runner beta dist-tag)
