...
1. **Auto-fixable linters** (guide says auto-fix supported): if a container engine is available, run `npx mega-linter-runner@beta --linter <KEY> --fix --release beta [failing files...]` (`--release beta` is required until MegaLinter v10 is released — standalone images are only multi-arch on `beta` before that; add `--container-engine podman` when using podman). Otherwise apply the underlying tool's documented fix behavior manually. If no engine is installed/running, ask the user first, then follow `container-engine.md` in the `megalinter-check` skill directory.
