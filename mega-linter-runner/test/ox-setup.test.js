...
describe("Ox setup", function () {
  it("(Ox setup) register", async () => {
    ...
    await oxSetup.run();
    await oxSetup.handleResponse({
      clientToken: oxSetup.clientToken,
      oxToken: dummyOxToken,
    });
    assert(
      fs.existsSync(OX_REPO_LOCAL_CONFIG_FILE),
      `Local config file does not exists`
    );
    assert(
      fs.readJSONSync(OX_REPO_LOCAL_CONFIG_FILE).oxToken === dummyOxToken,
      "Ox token is not good or missing"
    );
