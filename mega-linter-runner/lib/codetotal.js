  ...
  async run() {
    ...
    if (!fs.existsSync(path.join(process.cwd(), "docker-compose.yml"))) {
      const dockerComposeUrl =
        "https://raw.githubusercontent.com/oxsecurity/codetotal/main/docker-compose.yml";
      console.info(
        c.cyan(
          `Downloading latest docker-compose.yml from ${c.bold(dockerComposeUrl)} ...`,
        ),
      );
      https.get(dockerComposeUrl, (resp) =>
        resp.pipe(
          fs.createWriteStream(path.join(process.cwd(), "docker-compose.yml")),
        ),
      );
      ...
      console.log(
        "Running command: " +
          c.whiteBright(
            c.bgGray(
              "docker-compose " +
                ["-f", "docker-compose.yml", "pull"].join(" "),
            ),
          ),
      );
    ...
    console.log(
      "Running command: " +
        c.whiteBright(c.bgGray("docker-compose " + commandArgs.join(" "))),
    );
    const spawnOptions = {
      stdio: "inherit",
      windowsHide: true,
