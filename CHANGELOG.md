...
- CI
  ...
  - CI/Build performance:
    ...
    - Re-join PIPVENV and OTHER install blocks into chained `RUN` instructions (one RUN per package-manager block instead of one RUN per linter). The split granularity didn't pay off in practice: most cache invalidations come from changes near the end of the Dockerfile (Python source COPY), which leave all venv layers cached either way. The ~80 split layers added 30-60 s of per-layer GHA cache import/export overhead on every build for a partial-rebuild benefit that only triggered on Renovate version bumps. The chained form drops the main Dockerfile from ~80 to 35 RUN layers
    ...
    - In the DEV workflow, transport the built image to consumer jobs (test cases, run-all-code, runner tests, Trivy) via a `docker save` tarball with zstd-compressed layer blobs emitted directly by the BuildKit docker exporter (`compression=zstd,force-compression=true`) — drops the separate "zstd compress" step in the producer and the matching "zstd -d" decompress step in each of the four consumers, which now `docker load` the `.tar` directly. Registry-pushed images (ghcr.io BETA/RELEASE/ALPHA) keep the default gzip compression for broad Docker client compatibility.
    - DEV main image build is now read-only on the layer cache: it consumes `dev-main` and `beta-main` via `cache-from` but no longer writes `cache-to`, eliminating ~14 min of "exporting to GitHub Actions Cache" overhead per PR build. The warm cache that PRs read from is owned by the BETA workflow on `main`
    - Replace `cargo install` from source with prebuilt linux-x86_64 binary downloads from GitHub Releases for zizmor, stylua, sarif-fmt and shellcheck-sarif. Per-crate stages now take ~5-30 s instead of 5-13 min (zizmor alone was 13 min of jemalloc compilation), removing the dominant cold-build bottleneck. Prebuilt FROM+COPY snippets live in each linter's descriptor (`action`, `bash`, `lua`) and in `constants.py` for the universal `sarif-fmt`, so `.automation/build.py` stays agnostic of which crates ship as prebuilts. Each stage branches on `TARGETARCH`: amd64 downloads the prebuilt binary; arm64 falls back to compiling from source with Alpine's `rust`/`cargo`, since none of these projects publish an aarch64-linux build. `gcompat` is added to `DEFAULT_DOCKERFILE_APK_PACKAGES` so the glibc-linked prebuilt binaries run on the Alpine base in every image — including standalone per-linter images that previously inherited gcompat only via the kubernetes descriptor
