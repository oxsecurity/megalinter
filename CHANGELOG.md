...
- CI
  ...
  - CI/Build performance:
    ...
    - Replace `cargo install` from source with prebuilt linux-x86_64 binary downloads from GitHub Releases for zizmor, stylua, sarif-fmt and shellcheck-sarif. Per-crate stages now take ~5-30 s instead of 5-13 min (zizmor alone was 13 min of jemalloc compilation), removing the dominant cold-build bottleneck. Prebuilt FROM+COPY snippets live in each linter's descriptor (`action`, `bash`, `lua`) and in `constants.py` for the universal `sarif-fmt`, so `.automation/build.py` stays agnostic of which crates ship as prebuilts. Glibc binaries run on the Alpine runtime via the already-installed `gcompat` shim
