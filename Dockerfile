...
# Next FROM line commented because already managed by another linter
# FROM alpine/terragrunt:${TERRAFORM_TERRAGRUNT_VERSION} AS terragrunt
FROM rust:${RUST_RUST_VERSION}-alpine AS cargo-build-sarif-fmt
ARG CARGO_SARIF_FMT_VERSION
RUN apk add --no-cache build-base musl-dev openssl-dev openssl-libs-static pkgconfig bash perl
RUN cargo install --force --locked --root /out sarif-fmt@${CARGO_SARIF_FMT_VERSION}
FROM rust:${RUST_RUST_VERSION}-alpine AS cargo-build-zizmor
ARG CARGO_ZIZMOR_VERSION
RUN apk add --no-cache build-base musl-dev openssl-dev openssl-libs-static pkgconfig bash perl
RUN cargo install --force --locked --root /out zizmor@${CARGO_ZIZMOR_VERSION}
FROM rust:${RUST_RUST_VERSION}-alpine AS cargo-build-shellcheck-sarif
ARG CARGO_SHELLCHECK_SARIF_VERSION
RUN apk add --no-cache build-base musl-dev openssl-dev openssl-libs-static pkgconfig bash perl
RUN cargo install --force --locked --root /out shellcheck-sarif@${CARGO_SHELLCHECK_SARIF_VERSION}
FROM rust:${RUST_RUST_VERSION}-alpine AS cargo-build-stylua
ARG CARGO_STYLUA_VERSION
RUN apk add --no-cache build-base musl-dev openssl-dev openssl-libs-static pkgconfig bash perl
RUN cargo install --force --locked --root /out stylua@${CARGO_STYLUA_VERSION}
#FROM__END
...
COPY --link --from=cargo-build-sarif-fmt /out/bin/sarif-fmt /usr/bin/sarif-fmt
COPY --link --from=cargo-build-zizmor /out/bin/zizmor /usr/bin/zizmor
COPY --link --from=cargo-build-shellcheck-sarif /out/bin/shellcheck-sarif /usr/bin/shellcheck-sarif
COPY --link --from=cargo-build-stylua /out/bin/stylua /usr/bin/stylua
#COPY__END
