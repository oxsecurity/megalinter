...
RUN apk add --no-cache git=${APK_GIT_VERSION} && GOBIN=/usr/bin go install github.com/checkmarx/dustilock@v${REPOSITORY_DUSTILOCK_VERSION}
...
ARG APK_PERL_DEV_VERSION=5.40.1-r0
...
ARG APK_PY3_PYFLAKES_VERSION=3.2.0-r1
# renovate: datasource=pypi depName=ansible-lint
ARG PIP_ANSIBLE_LINT_VERSION=25.1.0
...
# renovate: datasource=repology depName=alpine_3_21/clang19-extra-tools
ARG APK_CLANG19_EXTRA_TOOLS_VERSION=19.1.4-r0
...
ARG NPM_ESLINT_VERSION=9.19.0
# renovate: datasource=npm depName=eslint-config-airbnb
ARG NPM_ESLINT_CONFIG_AIRBNB_VERSION=9.19.0
# renovate: datasource=npm depName=eslint-config-prettier
ARG NPM_ESLINT_CONFIG_PRETTIER_VERSION=19.0.4
...
ARG NPM_BABEL_CORE_VERSION=7.26.7
# renovate: datasource=npm depName=@babel/eslint-parser
ARG NPM_BABEL_ESLINT_PARSER_VERSION=7.26.5
...
ARG APK_OPENSSL_VERSION=3.3.2-r4
...
ARG GEM_RUBOCOP_VERSION=1.71.0
...
RUN apk add --no-cache \
                ...
                icu-libs=${APK_ICU_LIBS_VERSION} \
                go=${APK_GO_VERSION} \
                openjdk21=${APK_OPENJDK21_VERSION} \
                readline-dev=${APK_READLINE_DEV_VERSION} \
                perl=${APK_PERL_VERSION} \
                perl-dev=${APK_PERL_DEV_VERSION} \
...
RUN npm --no-cache install --ignore-scripts --omit=dev \
                ...
                eslint-plugin-node@${NPM_ESLINT_PLUGIN_NODE_VERSION} \
...
RUN ALPINE_GLIBC_BASE_URL="https://github.com/sgerrand/alpine-pkg-glibc/releases/download" && \
    ...
    apk add --no-cache --virtual=.build-dependencies wget ca-certificates=${APK_CA_CERTIFICATES_VERSION} && \
    echo \
        "-----BEGIN PUBLIC KEY-----\
...
#
# CSHARP installation
    && apk add --no-cache dotnet9-sdk=${APK_DOTNET9_SDK_VERSION}
